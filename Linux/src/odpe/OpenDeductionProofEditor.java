...
public class OpenDeductionProofEditor implements ActionListener, PropertyChangeListener, MouseListener, KeyListener{
	...
	private void subatomise() throws ProverException, IOException, MaudeException {
		...
		if (d == null){
			JOptionPane.showMessageDialog(frame, "Subatomic representation not currently supported.", "Oops...", JOptionPane.ERROR_MESSAGE);
			return;
		}
		subatomised = true;
	...
	private void interpret() throws ProverException, IOException, MaudeException {
		...
		if (d == null){
			JOptionPane.showMessageDialog(frame, "Interpretation not currently supported (or possible).", "Oops...", JOptionPane.ERROR_MESSAGE);
			return;
		}
		subatomised = false;
